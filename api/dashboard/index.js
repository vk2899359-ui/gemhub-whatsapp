...
<div id="app">
  <div id="sidebar">
    ...
    <div id="filterRow">
      ...
      <button class="chip" data-filter="call_interested" id="chipInterested">Interested</button>
...
(function () {
  ...
  var FILTERS = ['', 'unread', 'handed_over', 'ad_sourced', 'campaign_sourced', 'has_order', 'call_interested', 'call_not_interested', 'call_not_answering'];
  ...
  ['loginScreen','loginBox','pwInput','loginBtn','loginError','app','sidebar','convList',
   ...
   'backBtn','logoutBtn','chipInterested','chipNotInterested','chipNotAnswering'].forEach(function (id) { el[id] = document.getElementById(id); });
  ...
  // ---- call-tag chip counts ----
  function loadCallTagCounts() {
    api('/api/dashboard/call-tag-counts').then(function (r) {
      if (!r.ok || !r.json.counts) return;
      var c = r.json.counts;
      el.chipInterested.textContent = 'Interested (' + c.interested + ')';
  ...
  function startPolling() {
    ...
    clearInterval(state.countsTimer);
    state.countsTimer = setInterval(function () {
      if (document.hidden) return;
      loadCallTagCounts();
    }, 15000);
  }
