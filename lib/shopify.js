...
// ── Detailed product search (for image cards / native product messages) ──

function inferMetalAndCert(text, tags) {
  const hay = (String(text) + ' ' + (tags || []).join(' ')).toLowerCase();
  let metal = '';
  if (/vermeil/.test(hay)) metal = 'Gold vermeil';
  else if (/platinum|\b950\b/.test(hay)) metal = '950 Platinum';
  else if (/18k|18-k|18 k|\bgold\b/.test(hay)) metal = '18k gold';
  let cert = '';
  if (/\bigi\b/.test(hay)) cert = 'IGI-certified lab-grown diamond';
  else if (/\bgra\b/.test(hay) || /moissanite/.test(hay)) cert = 'GRA-certified moissanite';
  else if (/lab.?grown|lab.?diamond/.test(hay)) cert = 'IGI-certified lab-grown diamond';
  return { metal, cert };
}
...
function formatMoney(amount, code) {
  ...
  const sym = code === 'INR' ? '₹' : CONFIG.CURRENCY;
  return sym + n.toLocaleString('en-IN');
}

/**
 * Structured product search: returns real image, price, certification, metal
 * and URL for up to `limit` matching pieces. Never invents data.
 * @returns {Promise<Array<{id,title,handle,price,image,url,metal,cert,variantId,available}>>}
 */
export async function searchProductsDetailed(queryText, limit = 4) {
  const data = await graphql(
    `
    ...
    `,
    { q: queryText, n: Math.min(Math.max(limit, 1), 10) }
  );

  const edges = data?.products?.edges || [];
  return edges.map(({ node }) => {
    const price = node.priceRangeV2?.minVariantPrice;
    const inferred = inferMetalAndCert(`${node.title} ${node.productType || ''}`, node.tags);
    const image = node.featuredImage?.url || node.images?.edges?.[0]?.node?.url || null;
    ...
    return {
      ...
      title: node.title,
      handle: node.handle,
      price: price?.amount ? formatMoney(price.amount, price.currencyCode) : null,
      image,
      url,
      metal: inferred.metal,
      cert: inferred.cert,
      variantId: variant?.id || null,
      available: Boolean(variant?.availableForSale) && node.status === 'ACTIVE',
    };
  });
}

/** Catalogue-style caption for a product image message. */
export function productCaption(p) {
  const lines = [`*${p.title}*`];
  if (p.price) lines.push(p.price);
  const specs = [p.cert, p.metal].filter(Boolean).join(' · ');
