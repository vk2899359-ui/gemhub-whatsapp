...
export const TEMPLATE_NAMES = {
  ...
  LEAD_HANDOVER: 'gemhub_lead_handover',
};
...
/**
 * Sales-agent lead handover ping (used when the agent's 24h window is
 * closed). Vars: customer name, customer WhatsApp number, interest/topic,
 * handover reason. Kept short — full transcript is sent free-form once the
 * agent replies and the window opens.
 */
export function leadHandover({ customerName, customerPhone, interest, reason }) {
  return {
    name: TEMPLATE_NAMES.LEAD_HANDOVER,
    language: LANG,
    components: [
      {
        type: 'body',
        parameters: textParams(customerName, customerPhone, interest, reason),
      },
    ],
  };
}
