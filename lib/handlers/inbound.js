// Processes a single inbound WhatsApp message: de-dup, COD buttons, interactive
// reply routing (menus, booking slots, product picks) BY REPLY ID, the AI sales
// consultant (interactive replies), and human escalation.
import {
  sendText,
  markRead,
  sendButtons,
  sendList,
  sendLocation,
  sendCtaUrl,
  sendProductCards,
  recordInbound,
  downloadMediaAsBase64,
} from '../whatsapp.js';
import { CONFIG, secrets, isZohoConfigured } from '../env.js';
import { redis } from '../redis.js';
import { upsertZohoLead } from '../zoho.js';
import { fetchWithRetry } from '../http.js';
import { logIncoming, logSystem } from '../log.js';
import {
  advanceStage,
  setConversationName,
  setConversationCampaignSource,
  getConversationCampaignOffer,
  getConversationCampaignSource,
  recordOrderEvent,
} from '../conversations.js';
import {
  claimMessage,
  touchWindow,
  loadHistory,
  appendTurns,
  isEscalated,
  touchHandover,
  bumpIntentFailures,
  resetIntentFailures,
  isAdSourced,
  setAdSourced,
  getAdPlatform,
  setLastProducts,
  getLastProducts,
  setProfileName,
} from '../state.js';
import { getCodPending, clearCodPending } from '../stores.js';
import { addOrderTags, cancelOrder } from '../shopify.js';
import { generateReply } from '../claude.js';
import { isFromAgent, handleAgentInbound, triggerHandover, AGENT } from '../handover.js';
import { isOptOutMessage, optOutConfirmationMessage } from '../optout.js';
import { detectLanguage } from '../lang.js';
import { addSuppression, getLastCampaignSent, getRecipientStatus, setRecipientStatus, incrementCounter } from '../campaigns.js';

const S = CONFIG.SHOWROOM;

const HUMAN_REQUEST_RE =
  /\b(human|agent|representative|person|customer care|customer support|talk to (someone|a person)|call me|phone call|manager)\b|इंसान|व्यक्ति|एजेंट|बात कर(ना|ें)|फ़?ोन|call karo|baat kar|insaan/i;

// "Give me a number to call" — distinct from HUMAN_REQUEST_RE (which already
// catches "call me" / bare फ़ोन mentions); this catches asking FOR a number.
const NUMBER_REQUEST_RE =
  /\b(mobile|contact|whatsapp|business)?\s*number\b.{0,20}\b(please|share|send|do|give)\b|\b(share|send|give)\b.{0,10}\bnumber\b|\bmobile\s*no\.?\b|\bcontact\s*no\.?\b|number\s*(do|dedo|bhejo|batao)|नंबर\s*(दो|दे दो|भेजो|शेयर)/i;

// Safety net for things the bot genuinely can't do — the AI is prompted to
// call unable_to_help for these too, but this fast-path guarantees a
// deterministic handover even if the model misses it.
const BUSINESS_ESCALATION_RE =
  /\b(bulk order|wholesale|corporate order|corporate gifting|large order|complaint|refund|payment failed|payment issue|payment problem|fraud|scam|cheated|damaged item|defective item)\b|शिकायत|थोक\s*ऑर्डर|पेमेंट\s*(फेल|इशू|समस्या)|रिफंड|dhoka|paisa wapas|refund chahiye|payment fail/i;

const ESCALATION_THRESHOLD = 3;

const DEFAULT_MENU = [
  { id: 'browse_jewellery', title: 'Browse Jewellery' },
  { id: 'book_visit', title: 'Book Showroom Visit' },
  { id: 'custom_design', title: 'Custom Design' },
];
const AFTER_PRODUCTS_MENU = [
  { id: 'see_more', title: 'See More' },
  { id: 'book_visit', title: 'Book a Visit' },
  { id: 'talk_to_expert', title: 'Talk to Expert' },
];
const SLOTS = ['10:00 AM', '11:30 AM', '1:00 PM', '3:00 PM', '5:00 PM', '6:30 PM'];

// Directives fed to the AI when a menu button is tapped (routed by id). The
// consultant then replies in the customer's language using the right format.
const MENU_INTENT = {
  browse_jewellery:
    "[The customer tapped 'Browse Jewellery'. Before showing anything, ask ONE combined question covering BOTH budget range AND stone preference (solitaire lab-grown diamond / natural diamond / moissanite) in their language — e.g. \"Kis range mein dekh rahe hain, aur lab-grown diamond, natural diamond, ya moissanite — kya prefer karenge?\" Use both answers together to pick the right category and search — never show generic/unfiltered pieces first.]",
  custom_design:
    "[The customer tapped 'Custom Design'. Briefly explain custom & CAD design and ask what they have in mind — occasion, style, budget — in their language.]",
  see_more: "[The customer tapped 'See More'. Show a few more curated pieces in the same direction as before.]",
};

export async function handleInboundMessage(message, contact) {
  const messageId = message?.id;
  const from = message?.from;
  if (!from) return;

  const fresh = await claimMessage(messageId);
  if (!fresh) {
    await logSystem({ event: 'duplicate_ignored', messageId });
    return;
  }

  const profileName = contact?.profile?.name || 'there';
  await touchWindow(from);
  markRead(messageId);
  if (contact?.profile?.name) {
    await setProfileName(from, contact.profile.name);
    await setConversationName(from, contact.profile.name);
  }
  await recordInbound(from, message);
  await tagCampaignReplyIfAny(from);
  await pushLeadToZohoIfNew(from, contact?.profile?.name);
  await refreshZohoLeadDescription(from, contact?.profile?.name);

  const button = extractButtonPayload(message); // CONFIRM / CANCEL only
  const interactiveId = extractInteractiveId(message);
  const text = extractText(message);

  await logIncoming({ from, messageId, type: message?.type, button, interactiveId, text, profileName });

  // 0) Messages from the sales agent (Keshav) are commands, never customer
  // chat — route separately and stop, regardless of anything else below.
  if (isFromAgent(from)) {
    return handleAgentInbound(message);
  }

  // 1) COD quick-reply buttons take priority.
  if (button === 'CONFIRM' || button === 'CANCEL') {
    return handleCodButton(from, button, profileName);
  }

  // 1b) Opt-out is mandatory and always honoured — registers and confirms
  // even if the bot is otherwise quiet (handover, etc). Suppresses future
  // CAMPAIGN sends only; does not disable support chat.
  if (text && isOptOutMessage(text)) {
    return handleOptOut(from, text);
  }

  // 1c) WhatsApp's native Catalog + Cart "Place order" — always handled
  // regardless of bot on/off state. We never collect payment in-chat (see
  // system prompt hard rules), so this hands straight to a human rather
  // than letting the AI try to improvise a checkout.
  if (message?.type === 'order' && message.order) {
    return handleCartOrder(from, profileName, message.order);
  }

  // 2) If a human is already handling this chat, stay out of the way — and
  // slide the 24h auto-release window forward since the customer is active.
  if (await isEscalated(from)) {
    await touchHandover(from);
    await logSystem({ event: 'inbound_while_escalated', from });
    return;
  }

  // 3) Native interactive replies — routed by reply id, not by matching text.
  if (interactiveId) {
    const routed = await routeInteractive(from, interactiveId, message);
    if (routed.handled) return;
    if (routed.aiText) return handleAiChat(from, routed.aiText);
  }

  // 4) Photo message — let the AI actually look at it (style match, "do you
  // have this?") instead of asking the customer to re-type as text.
  if (message?.type === 'image' && message.image?.id) {
    return handleImageMessage(from, profileName, message.image, text);
  }

  // 4b) Nothing textual to answer (other media, reaction, etc.).
  if (!text) {
    await sendText(
      from,
      `Thanks ${profileName}! 🙏 Could you type your question so I can help? You can also visit our Sector 45, Gurugram showroom (${S.hours}) or email ${S.email}.`
    );
    return;
  }

  // 5) Explicit request for a person -> handover immediately.
  if (HUMAN_REQUEST_RE.test(text)) {
    return escalate(from, 'explicit_human_request', { profileName, triggerText: text });
  }

  // 5b) Asking for a number to call -> handover immediately.
  if (NUMBER_REQUEST_RE.test(text)) {
    return escalate(from, 'number_request', { profileName, triggerText: text });
  }

  // 5c) Bulk/wholesale/complaint/payment-issue safety net -> handover.
  if (BUSINESS_ESCALATION_RE.test(text)) {
    return escalate(from, 'business_escalation', { profileName, triggerText: text });
  }

  // 6) Ad-sourced first-touch: tag so we open with a qualifying question.
  // A `referral` object is only present on Meta's own Facebook/Instagram
  // click-to-WhatsApp ads (Meta populates it server-side) — Google's
  // click-to-WhatsApp ads use the same prefilled-text mechanism but never
  // carry this field, so its presence/absence is what actually tells the
  // two platforms apart (matching the starter text alone can't).
  if (message?.referral) await setAdSourced(from, 'meta');
  else if (isAdStarter(text)) await setAdSourced(from, 'google');

  // 7) AI chat.
  return handleAiChat(from, text);
}

// ── Interactive reply routing (by id) ────────────────────────────────

async function routeInteractive(from, id, message) {
  // Booking slot picked from the time-slot list.
  if (id.startsWith('slot:')) {
    const slot = id.slice('slot:'.length);
    await sendText(from, `Booked! ✅ ${slot} — GemHub showroom, Sector 45, Gurugram. See you soon. 💎`);
    await sendLocation(from, {
      latitude: S.latitude,
      longitude: S.longitude,
      name: S.name,
      address: S.address,
    });
    await sendCtaUrl(from, `We're open ${S.hours}. Tap for directions:`, 'Get Directions', S.mapsUrl);
    await appendTurns(from, [
      { role: 'user', content: `[Booked a showroom visit: ${slot}]` },
      { role: 'assistant', content: `[Confirmed visit for ${slot} and shared the showroom location]` },
    ]);
    await advanceStage(from, 'booked');
    await logSystem({ event: 'visit_booked', from, slot });
    return { handled: true };
  }

  // A piece picked from a product shortlist.
  if (id.startsWith('product:')) {
    const pid = id.slice('product:'.length);
    const shortlist = await getLastProducts(from);
    const piece = shortlist.find((p) => p.id === pid);
    if (piece) {
      await sendProductCards(from, [piece]);
      await sendButtons(from, 'Love it? I can hold it for a viewing or share more like it.', AFTER_PRODUCTS_MENU);
      await appendTurns(from, [
        { role: 'user', content: `[Selected piece: ${piece.title}]` },
        { role: 'assistant', content: `[Showed ${piece.title} with a View Product link]` },
      ]);
      await advanceStage(from, 'shown_products');
      return { handled: true };
    }
    return { aiText: '[The customer selected a piece from the shortlist. Show it again with details and offer to book a viewing.]' };
  }

  // Menu buttons.
  if (id === 'talk_to_expert') {
    await escalate(from, 'talk_to_expert');
    return { handled: true };
  }
  if (id === 'get_directions') {
    await sendText(from, `Here's our showroom — we'd love to see you. 💎`);
    await sendLocation(from, {
      latitude: S.latitude,
      longitude: S.longitude,
      name: S.name,
      address: S.address,
    });
    await sendCtaUrl(from, `Open ${S.hours}. Tap for directions:`, 'Get Directions', S.mapsUrl);
    return { handled: true };
  }
  if (id === 'book_visit' || id === 'book_a_visit') {
    await sendBooking(from, 'Wonderful — pick a time that suits you and I’ll note it down. 💎');
    return { handled: true };
  }
  if (MENU_INTENT[id]) return { aiText: MENU_INTENT[id] };

  // Unknown id — hand the tapped title to the AI.
  const title =
    message?.interactive?.button_reply?.title || message?.interactive?.list_reply?.title || id;
  return { aiText: `[Customer tapped: ${title}]` };
}

// ── COD confirm / cancel ─────────────────────────────────────────────

// ── Campaign reply attribution ────────────────────────────────────────
// If this customer was messaged by a campaign recently, tag the CRM
// conversation with it and count the reply once (idempotent per campaign
// recipient via the repliedAt guard) — feeds both the dashboard's
// "campaign-sourced" context and the campaign's own analytics.

async function tagCampaignReplyIfAny(from) {
  try {
    const campaignId = await getLastCampaignSent(from);
    if (!campaignId) return;
    const recipient = await getRecipientStatus(campaignId, from);
    await setConversationCampaignSource(from, campaignId, recipient?.offerText);
    if (recipient && !recipient.repliedAt) {
      await setRecipientStatus(campaignId, from, { repliedAt: Date.now() });
      await incrementCounter(campaignId, 'replied');
    }
  } catch (err) {
    await logSystem({ event: 'campaign_reply_tag_error', from, error: err.message });
  }
}

// ── WhatsApp -> Zoho lead push ────────────────────────────────────────
// A phone number becomes a Zoho Lead once it's shown genuine engagement —
// ENGAGEMENT_THRESHOLD or more customer-sent messages — not on the very
// first message. Found live: pushing on message #1 (plus a one-time
// backfill that, before a separate fix, counted cold campaign-blast
// touches as "conversations") flooded Zoho with hundreds of one-word-reply
// and zero-reply "leads". A permanent NX flag still gates a phone from
// ever being pushed twice. Best-effort: a Zoho outage must never block the
// customer's WhatsApp reply, so failures are logged and the flag is rolled
// back to retry on their next message, rather than surfaced to the
// conversation flow.
async function pushLeadToZohoIfNew(from, profileName) {
  if (!isZohoConfigured()) return;
  const flagKey = `zoho:lead:pushed:${from}`;
  try {
    const already = await redis().get(flagKey);
    if (already) return; // already pushed

    const claimed = await redis().set(flagKey, '1', { nx: true });
    if (claimed !== 'OK') return; // a concurrent message already claimed it
    try {
      const source = await resolveLeadSource(from);
      const domestic = isDomesticPhone(from);
      const description = await buildLeadDescription(from);
      await upsertZohoLead({ phone: from, name: profileName || '', source, domestic, description });
      await logSystem({ event: 'zoho_lead_pushed', from, source, domestic });
    } catch (err) {
      await redis().del(flagKey);
      throw err;
    }
  } catch (err) {
    await logSystem({ event: 'zoho_lead_push_error', from, error: err.message });
  }
}

// The first message is very often the generic ad-click opener ("Hello! Can
// I get more info on this?"), which is useless as a CRM requirement note —
// unlike IndiaMART leads, which arrive with the customer's actual filled-in
// requirement text. A real conversation's actual interest is often spread
// across several SHORT messages ("5000-8000 budget", "ring chahiye", "gold
// color") rather than one long one, so picking "the longest single message"
// misses it — only a real read of the conversation gets this right. Genuine
// single-message drop-offs (the large majority of ad clicks) are left as
// their literal text — no need to summarize one line into another line.
async function buildLeadDescription(phone) {
  const rawThread = await redis().lrange(`thread:${phone}`, 0, 19);
  const inboundTexts = [];
  const turns = [];
  for (const raw of rawThread) {
    let msg;
    try {
      msg = typeof raw === 'object' ? raw : JSON.parse(raw);
    } catch {
      continue;
    }
    const body = (msg.body || '').trim();
    if (msg.dir === 'in' && msg.kind === 'text' && body) inboundTexts.push(body);
    if (body) turns.push(`${msg.dir === 'in' ? 'Customer' : 'GemHub'}: ${body}`.slice(0, 400));
  }

  if (inboundTexts.length === 0) return 'WhatsApp enquiry — no details shared yet.';
  if (inboundTexts.length === 1) return `WhatsApp enquiry: "${inboundTexts[0].slice(0, 300)}"`;

  try {
    return await summarizeLeadInterest(turns.slice(0, 20));
  } catch (err) {
    await logSystem({ event: 'zoho_description_summarize_error', phone, error: err.message });
    // Fall back to the customer's first real message rather than nothing.
    return `WhatsApp enquiry: "${inboundTexts[0].slice(0, 300)}"`;
  }
}

/** One short line, CRM-requirement-note style, from a short chat transcript. */
async function summarizeLeadInterest(turns) {
  const res = await fetchWithRetry(
    'https://api.anthropic.com/v1/messages',
    {
      method: 'POST',
      headers: {
        'x-api-key': secrets.ANTHROPIC_API_KEY,
        'anthropic-version': '2023-06-01',
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        model: CONFIG.ANTHROPIC_MODEL,
        max_tokens: 100,
        system:
          'Write ONE short CRM requirement-note line (like "Requirement for X: details") summarizing what this customer is asking for, based on their WhatsApp chat with a jewellery store. State only what they actually said — product/style, budget, occasion, metal, if mentioned. No greeting, no quotes, no explanation, just the one line. If nothing concrete was said yet, write "No specific requirement shared yet."',
        messages: [{ role: 'user', content: turns.join('\n') }],
      }),
    },
    { attempts: 2, baseDelayMs: 500, label: 'anthropic:lead-description' }
  );
  const json = await res.json();
  if (!res.ok) throw new Error(`Anthropic error (${res.status}): ${json?.error?.message || JSON.stringify(json)}`);
  const text = (json.content || [])
    .filter((b) => b.type === 'text')
    .map((b) => b.text)
    .join(' ')
    .trim();
  return text || 'No specific requirement shared yet.';
}

/** Best-known channel this lead came through, in Zoho's Lead_Source terms. */
async function resolveLeadSource(from) {
  const campaignId = await getConversationCampaignSource(from);
  if (campaignId) return 'WhatsApp Campaign';
  const adPlatform = await getAdPlatform(from);
  if (adPlatform === 'meta') return 'Facebook/Instagram Ads';
  if (adPlatform === 'google') return 'Google Ads';
  return 'Organic - WhatsApp';
}

// One-time follow-up sync: once a real back-and-forth starts (2nd inbound
// message — push happens on message 1, before there's anything to
// summarize), push a richer Description to the already-created Zoho lead.
// Gated by its own flag so this only ever calls Zoho (and the summarizer)
// once per contact, not on every subsequent message.
async function refreshZohoLeadDescription(from, profileName) {
  if (!isZohoConfigured()) return;
  try {
    const pushed = await redis().get(`zoho:lead:pushed:${from}`);
    if (!pushed) return; // not a Zoho lead (yet) — pushLeadToZohoIfNew handles the first write
    const richFlagKey = `zoho:lead:descrich:${from}`;
    if (await redis().get(richFlagKey)) return; // already refreshed once

    const thread = await redis().lrange(`thread:${from}`, 0, 19);
    const inboundCount = thread.reduce((n, raw) => {
      try {
        const msg = typeof raw === 'object' ? raw : JSON.parse(raw);
        return msg.dir === 'in' && msg.kind === 'text' ? n + 1 : n;
      } catch {
        return n;
      }
    }, 0);
    if (inboundCount < 2) return; // still just the opener — nothing new to summarize

    const claimed = await redis().set(richFlagKey, '1', { nx: true });
    if (claimed !== 'OK') return;
    const source = await resolveLeadSource(from);
    const domestic = isDomesticPhone(from);
    const description = await buildLeadDescription(from);
    await upsertZohoLead({ phone: from, name: profileName || '', source, domestic, description });
    await logSystem({ event: 'zoho_lead_description_refreshed', from });
  } catch (err) {
    await logSystem({ event: 'zoho_lead_description_refresh_error', from, error: err.message });
  }
}

// India's country code is exactly '91' (a bare 10-digit number is normalized
// to 91 + 10 digits = 12 total, see lib/phone.js) — no other country's
// calling code starts this way, so this check is unambiguous.
function isDomesticPhone(phone) {
  return /^91\d{10}$/.test(phone || '');
}

// ── Opt-out (mandatory, campaign suppression) ─────────────────────────

async function handleOptOut(from, triggerText) {
  await addSuppression(from, { via: 'inbound_message' });
  const lang = detectLanguage(triggerText);
  await sendText(from, optOutConfirmationMessage(lang));
  const campaignId = await getLastCampaignSent(from);
  if (campaignId) await incrementCounter(campaignId, 'optedOut');
  await logSystem({ event: 'opted_out', from, triggerText, campaignId: campaignId || null });
}

async function handleCartOrder(from, profileName, order) {
  const items = order.product_items || [];
  const total = items.reduce((sum, it) => sum + (Number(it.item_price) || 0) * (Number(it.quantity) || 0), 0);
  const itemCount = items.reduce((sum, it) => sum + (Number(it.quantity) || 0), 0);
  const totalFmt = total.toLocaleString('en-IN');

  await recordOrderEvent(from, {
    kind: 'cart_order_placed',
    body: `WhatsApp cart order — ${itemCount} item(s), ₹${totalFmt}`,
  });

  await sendText(
    from,
    `Thank you, ${profileName}! 🙏 We've received your order (₹${totalFmt}). Our team will confirm availability and share a secure payment link shortly — we never take payment details over chat. For anything urgent, call/WhatsApp us at ${S.phone}.`
  );

  await escalate(from, 'cart_order_placed', {
    profileName,
    triggerText: `WhatsApp cart order: ${itemCount} item(s), ₹${totalFmt}`,
  });
  await logSystem({ event: 'whatsapp_cart_order', from, itemCount, total, items });
}

async function handleCodButton(from, decision, profileName) {
  const pending = await getCodPending(from);
  if (!pending) {
    await sendText(
      from,
      `Thanks ${profileName}! I couldn’t find a pending order to update — it may already be confirmed. For help, reply here or email ${S.email}.`
    );
    return;
  }

  try {
    if (decision === 'CONFIRM') {
      await addOrderTags(pending.orderId, ['cod-confirmed']);
      await clearCodPending(from);
      await sendText(
        from,
        `Thank you! ✅ Your COD order ${pending.orderName} is confirmed. We’re packing it now and you’ll get a tracking link once it ships. 💎`
      );
      await logSystem({ event: 'cod_confirmed', order: pending.orderName, from });
    } else {
      await addOrderTags(pending.orderId, ['cod-cancelled']);
      await cancelOrder(pending.orderId, { reason: 'customer' });
      await clearCodPending(from);
      await sendText(
        from,
        `Your COD order ${pending.orderName} has been cancelled. No charge applies. If this was a mistake or you’d like help ordering again, just reply here. 🙏`
      );
      await logSystem({ event: 'cod_cancelled', order: pending.orderName, from });
    }
  } catch (err) {
    await logSystem({ event: 'cod_button_error', from, decision, error: err.message });
    await sendText(
      from,
      `Sorry, something went wrong updating your order. Our team will sort it out — please email ${S.email} and we’ll fix it right away. 🙏`
    );
  }
}

// ── AI chat ──────────────────────────────────────────────────────────

// ── Photo messages ──────────────────────────────────────────────────

async function handleImageMessage(from, profileName, imageMeta, captionText) {
  let image;
  try {
    image = await downloadMediaAsBase64(imageMeta.id);
  } catch (err) {
    await logSystem({ event: 'image_download_error', from, error: err.message });
    await sendText(
      from,
      `Thanks for the photo, ${profileName}! I couldn't quite load it on my end 🙏 Could you try sending it again, or describe the piece in words?`
    );
    return;
  }
  return handleAiChat(from, captionText || imageMeta.caption || '', { image });
}

async function handleAiChat(from, text, { image } = {}) {
  // What actually goes into stored history — the raw base64 image itself
  // never should (wasteful to keep and to re-send on every future turn), so
  // a photo turn is remembered as a short marker instead.
  const historyText = image ? `[Sent a photo]${text ? ': ' + text : ''}` : text;

  let reply;
  try {
    const adSourced = await isAdSourced(from);
    const campaignOffer = await getConversationCampaignOffer(from);
    const history = await loadHistory(from);
    reply = await generateReply({ history, userText: text, adSourced, campaignOffer, image });
  } catch (err) {
    await logSystem({ event: 'ai_error', from, error: err.message });
    await sendText(
      from,
      `Sorry, I’m having a moment 🙏 Please try again, or reach us at ${S.email} / our Sector 45, Gurugram showroom (${S.hours}).`
    );
    return;
  }

  // Escalation with nothing to render -> hand off.
  if (reply.wantsHuman && reply.actions.length === 0) {
    await escalate(from, reply.reason || 'human_requested', { triggerText: text });
    await appendTurns(from, [
      { role: 'user', content: historyText },
      { role: 'assistant', content: `[Handed off to ${AGENT.name}, GemHub sales specialist]` },
    ]);
    return;
  }

  const summary = await renderActions(from, reply.actions);

  await appendTurns(from, [
    { role: 'user', content: historyText },
    { role: 'assistant', content: summary || '[sent options]' },
  ]);

  // Escalation logic (soft failures accumulate).
  if (reply.unableCount > 0) {
    const fails = await bumpIntentFailures(from, reply.unableCount);
    if (fails >= ESCALATION_THRESHOLD) {
      await escalate(from, `failed_intents:${fails}`, { triggerText: text });
    }
  } else {
    await resetIntentFailures(from);
  }
}

// ── Render actions from the consultant ───────────────────────────────

async function renderActions(from, actions) {
  const bodies = [];
  for (const action of actions || []) {
    if (action.body) bodies.push(action.body);
    switch (action.kind) {
      case 'text':
        await sendText(from, action.body);
        break;
      case 'menu':
        await sendButtons(from, action.body, DEFAULT_MENU);
        break;
      case 'after_products':
        await sendButtons(from, action.body, AFTER_PRODUCTS_MENU);
        break;
      case 'location':
        if (action.body) await sendText(from, action.body);
        await sendLocation(from, {
          latitude: S.latitude,
          longitude: S.longitude,
          name: S.name,
          address: S.address,
        });
        await sendCtaUrl(from, `We're open ${S.hours}. Tap for directions:`, 'Get Directions', S.mapsUrl);
        break;
      case 'booking':
        await sendBooking(from, action.body);
        break;
      case 'products':
        await sendProductCards(from, action.products, { body: action.body });
        await setLastProducts(from, action.products);
        await advanceStage(from, 'shown_products');
        break;
      default:
        if (action.body) await sendText(from, action.body);
    }
  }
  return bodies.join('\n').trim();
}

async function sendBooking(from, body) {
  await sendList(
    from,
    body || 'Pick a time that suits you:',
    'Pick a time',
    [
      {
        title: 'Today & this week',
        rows: SLOTS.map((s) => ({
          id: `slot:${s}`,
          title: s,
          description: 'Showroom visit, Sector 45 Gurugram',
        })),
      },
    ],
    { header: 'Book your visit', footer: `Open ${S.hours}` }
  );
}

// ── Escalation / human handover ──────────────────────────────────────
// Thin wrapper: the real work (language-aware customer message, notifying
// the sales agent, and the Redis handover flag) lives in lib/handover.js.

async function escalate(from, reason, { profileName, triggerText } = {}) {
  await triggerHandover(from, { reason, profileName, triggerText });
}

// ── WhatsApp payload extraction ──────────────────────────────────────

function extractText(message) {
  if (message?.type === 'text') return (message.text?.body || '').trim();
  if (message?.type === 'interactive') {
    const it = message.interactive;
    if (it?.type === 'list_reply') return (it.list_reply?.title || '').trim();
    if (it?.type === 'button_reply') return (it.button_reply?.title || '').trim();
  }
  if (message?.type === 'button') return (message.button?.text || '').trim();
  return '';
}

/** Native interactive reply id (button_reply.id or list_reply.id), or null. */
function extractInteractiveId(message) {
  if (message?.type !== 'interactive') return null;
  const it = message.interactive;
  if (it?.type === 'button_reply') return it.button_reply?.id || null;
  if (it?.type === 'list_reply') return it.list_reply?.id || null;
  return null;
}

/**
 * Returns 'CONFIRM' | 'CANCEL' | null. Template quick-reply buttons arrive as
 * type 'button' with a payload; native interactive replies as button_reply.id.
 */
function extractButtonPayload(message) {
  let raw = null;
  if (message?.type === 'button') {
    raw = message.button?.payload || message.button?.text;
  } else if (message?.type === 'interactive' && message.interactive?.type === 'button_reply') {
    raw = message.interactive.button_reply?.id || message.interactive.button_reply?.title;
  }
  if (!raw) return null;
  const up = String(raw).trim().toUpperCase();
  if (up.includes('CONFIRM')) return 'CONFIRM';
  if (up.includes('CANCEL')) return 'CANCEL';
  return null;
}

function isAdStarter(text) {
  const t = (text || '').toLowerCase();
  return /gemhub/.test(t) && /lab-?grown diamond/.test(t);
}
