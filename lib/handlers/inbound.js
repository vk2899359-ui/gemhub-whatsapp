...
// Every phone number that messages the bot becomes a Zoho Lead, pushed once
// (a permanent NX flag gates repeat messages from re-hitting the Zoho API).
// Best-effort: a Zoho outage must never block the customer's WhatsApp reply,
// so failures are logged and the flag is rolled back to retry on their next
// message, rather than surfaced to the conversation flow.
async function pushLeadToZohoIfNew(from, profileName) {
  ...
  try {
    ...
    if (claimed !== 'OK') return; // already pushed
    try {
      await upsertZohoLead({ phone: from, name: profileName || '' });
      await logSystem({ event: 'zoho_lead_pushed', from });
    } catch (err) {
