...
export async function handleInboundMessage(message, contact) {
  ...
  // 5) Explicit request for a person -> escalate immediately.
  if (HUMAN_REQUEST_RE.test(text)) {
    return escalate(from, 'explicit_human_request');
  }
...
async function handleAiChat(from, text) {
  ...
  if (reply.wantsHuman && reply.actions.length === 0) {
    await escalate(from, reply.reason || 'human_requested');
    await appendTurns(from, [
      { role: 'user', content: text },
      { role: 'assistant', content: '[Handed off to a GemHub specialist]' },
    ]);
    return;
  }
...
async function escalate(from, reason, { alreadyReplied = false } = {}) {
  ...
  if (!alreadyReplied) {
    // WhatsApp free-form messages can't render a tel: CTA button, so we surface
    // the number in text (WhatsApp makes it tap-to-call automatically).
    await sendText(
      from,
      `Of course — I’m connecting you with a GemHub specialist who’ll reply here shortly. 🙏\n📞 Prefer to talk now? Call us: ${S.phone}\n📍 ${S.address} (${S.hours}) · ${S.email}`
    );
  }
}
