...
// conversation flow.
const ZOHO_ENGAGEMENT_THRESHOLD = 4;
...
async function pushLeadToZohoIfNew(from, profileName) {
  ...
  try {
    ...
    // A reply to one of our own WhatsApp campaigns is already a strong
    // intent signal (they chose to respond to marketing) — push it to Zoho
    // on the very first reply, skipping the engagement wait that exists to
    // filter out one-word/no-reply organic and ad-sourced noise.
    const campaignId = await getConversationCampaignSource(from);
    let inboundCount = null;
    if (!campaignId) {
      const thread = await redis().lrange(`thread:${from}`, 0, -1);
      inboundCount = thread.reduce((n, raw) => {
        try {
          const msg = typeof raw === 'object' ? raw : JSON.parse(raw);
          return msg.dir === 'in' ? n + 1 : n;
        } catch {
          return n;
        }
      }, 0);
      if (inboundCount < ZOHO_ENGAGEMENT_THRESHOLD) return; // not enough engagement yet
    }
    ...
    try {
      ...
      const tags = [isDomesticPhone(from) ? 'Domestic' : 'International'];
      await upsertZohoLead({ phone: from, name: profileName || '', source, tags });
      await logSystem({ event: 'zoho_lead_pushed', from, inboundCount, source, tags });
    } catch (err) {
