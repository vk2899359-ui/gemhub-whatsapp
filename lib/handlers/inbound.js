...
import { CONFIG, isZohoConfigured } from '../env.js';
...
// requirement text. Budget/occasion (already extracted by recordMessage
// into conv:meta) rarely land in message 1 either. So the description is
// built once at push time from whatever's known then, and re-synced once
// later when budget/occasion first appear — same "once, cheaply" shape as
// the push flag itself, not a per-message Zoho call.
async function buildLeadDescription(phone) {
  const meta = await redis().hgetall(`conv:meta:${phone}`);
  const budget = meta?.leadBudget != null ? String(meta.leadBudget) : null;
  const occasion = meta?.leadOccasion != null ? String(meta.leadOccasion) : null;
  if (budget || occasion) {
    const parts = [];
    if (occasion) parts.push(`Occasion: ${occasion}`);
    if (budget) parts.push(`Budget: ${budget}`);
    return `WhatsApp enquiry — ${parts.join(', ')}.`;
  }

  const thread = await redis().lrange(`thread:${phone}`, 0, 9);
  let best = '';
  for (const raw of thread) {
    ...
    if (msg.dir !== 'in' || msg.kind !== 'text') continue;
    const body = (msg.body || '').trim();
    // Skip short generic openers ("Hi", "Hello! Can I get more info on
    // this?") in favour of the first message that actually says something.
    if (body.length > best.length && body.length >= 15) best = body;
  }
  if (best) return `WhatsApp enquiry: "${best.slice(0, 300)}"`;
  return 'WhatsApp enquiry — no details shared yet.';
}
...
// One-time follow-up sync: once budget/occasion first become known (usually
// a few messages in, not message 1), push a richer Description to the
// already-created Zoho lead. Gated by its own flag so this only ever calls
// Zoho once per contact, not on every subsequent message.
async function refreshZohoLeadDescription(from, profileName) {
  ...
  try {
    ...
    const meta = await redis().hgetall(`conv:meta:${from}`);
    const hasSignal = meta?.leadBudget != null || meta?.leadOccasion != null;
    if (!hasSignal) return;
