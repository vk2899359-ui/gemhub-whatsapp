...
// Maps a WhatsApp Graph API send body into our CRM thread-entry shape (see
// lib/conversations.js). Used to populate the dashboard's per-conversation
// message thread from the SAME call site every send already goes through,
// so nothing sent via any flow (AI, COD, cron, dashboard) is ever missed.
function describeOutbound(body) {
  switch (body.type) {
    case 'text':
      return { kind: 'text', body: body.text?.body || '' };
    case 'template': {
      const bodyComp = body.template?.components?.find((c) => c.type === 'body');
      const params = (bodyComp?.parameters || []).map((p) => p.text).filter(Boolean);
      return { kind: 'template', templateName: body.template?.name, body: params.join(' · ') };
    }
    case 'image':
      return { kind: 'image', mediaUrl: body.image?.link, caption: body.image?.caption || '' };
    case 'location':
      return { kind: 'location', location: body.location };
    case 'interactive': {
      ...
      if (it.type === 'cta_url') {
        return {
          kind: 'cta_url',
          body: it.body?.text || '',
          cta: { text: it.action?.parameters?.display_text, url: it.action?.parameters?.url },
        };
      }
      if (it.type === 'product' || it.type === 'product_list') {
        return { kind: it.type, body: it.body?.text || '[Product card]' };
      }
      return { kind: 'interactive', body: '' };
    }
    default:
      return { kind: body.type || 'unknown', body: '' };
  }
}
...
async function callGraph(body, { label } = {}) {
  ...
  if (!res.ok) {
    ...
    // Structured Meta error info for callers that need to classify failures
    // (e.g. the campaign sender's retry/backoff logic). Existing callers are
    // unaffected — they only ever read err.message.
    err.httpStatus = res.status;
    err.metaCode = json?.error?.code ?? null;
    err.metaSubcode = json?.error?.error_subcode ?? null;
    throw err;
  }
  ...
  try {
    await recordMessage(body.to, { dir: 'out', wamid: messageId, ...describeOutbound(body) });
  } catch (err) {
    console.error('[recordMessage:out] failed', err?.message);
  }
...
/**
 * Record an inbound WhatsApp message into the CRM thread. Call once per
 * webhook message, before any bot logic. No-op for the sales agent's own
 * number (handled inside conversations.recordMessage).
 */
export async function recordInbound(from, message) {
  try {
    await recordMessage(from, { dir: 'in', wamid: message?.id, ...describeInbound(message) });
  } catch (err) {
    console.error('[recordMessage:in] failed', err?.message);
  }
}

function describeInbound(message) {
  const type = message?.type;
  if (type === 'text') return { kind: 'text', body: message.text?.body || '' };
  if (type === 'interactive') {
    const it = message.interactive;
    if (it?.type === 'button_reply') {
      return { kind: 'button_reply', body: it.button_reply?.title || '', tappedId: it.button_reply?.id };
    }
    if (it?.type === 'list_reply') {
      return { kind: 'list_reply', body: it.list_reply?.title || '', tappedId: it.list_reply?.id };
    }
  }
  if (type === 'button') {
    return { kind: 'template_button_reply', body: message.button?.text || '', tappedId: message.button?.payload };
  }
  if (type === 'image') return { kind: 'image', mediaId: message.image?.id || '', caption: message.image?.caption || '' };
  if (type === 'video') return { kind: 'video', mediaId: message.video?.id || '', caption: message.video?.caption || '' };
  if (type === 'audio') return { kind: 'audio', mediaId: message.audio?.id || '' };
  if (type === 'document') {
    return {
      kind: 'document',
  ...
  if (type === 'sticker') return { kind: 'sticker', mediaId: message.sticker?.id || '' };
  if (type === 'location') return { kind: 'location', location: message.location };
  if (type === 'order' && message.order) {
    ...
    return {
      kind: 'order',
      body: `WhatsApp cart order — ${itemCount} item(s), ₹${total.toLocaleString('en-IN')}`,
      orderItems: items,
      orderTotal: total,
    };
  }
  return { kind: type || 'unknown', body: '' };
}

const MAX_INBOUND_MEDIA_BYTES = 5 * 1024 * 1024; // Claude vision's own per-image limit

/**
 * Download an inbound media attachment (image, etc.) and return it ready to
 * hand to Claude's vision input. WhatsApp only ever gives us a media *id* on
 * the inbound message — the actual bytes live behind a short-lived,
 * auth-gated URL that has to be resolved first.
...
export async function downloadMediaAsBase64(mediaId) {
  const metaRes = await fetchWithRetry(
    graphUrl(mediaId),
    { headers: { Authorization: `Bearer ${secrets.META_TOKEN}` } },
    { attempts: 3, baseDelayMs: 500, label: 'wa:media_lookup' }
  );
  const metaJson = await metaRes.json();
  if (!metaRes.ok || !metaJson.url) {
    throw new Error(`Media lookup failed (${metaRes.status}): ${JSON.stringify(metaJson?.error || metaJson).slice(0, 300)}`);
  }
  if (metaJson.file_size && metaJson.file_size > MAX_INBOUND_MEDIA_BYTES) {
    throw new Error(`Media too large (${metaJson.file_size} bytes)`);
  }

  const fileRes = await fetchWithRetry(
    metaJson.url,
    { headers: { Authorization: `Bearer ${secrets.META_TOKEN}` } },
    { attempts: 3, baseDelayMs: 500, label: 'wa:media_download' }
  );
  if (!fileRes.ok) throw new Error(`Media download failed (${fileRes.status})`);
  const buffer = Buffer.from(await fileRes.arrayBuffer());
  if (buffer.length > MAX_INBOUND_MEDIA_BYTES) throw new Error(`Media too large (${buffer.length} bytes)`);

  return { base64: buffer.toString('base64'), mimeType: metaJson.mime_type || 'image/jpeg' };
}
