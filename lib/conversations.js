...
export async function searchConversations({ q, filter, limit = 100 } = {}) {
  ...
  else if (filter === 'called_by_sonia') items = items.filter((c) => c.calledBy === 'Sonia');
  else if (filter === 'called_by_reshma') items = items.filter((c) => c.calledBy === 'Reshma');

  return items.slice(0, limit);
}

/**
 * Counts of each call-outcome tag (plus per-agent call totals), for the
 * filter chip labels. Scanned from the same bounded pool as search/filter —
 * fine at this business's scale.
 */
export async function getCallTagCounts() {
  const r = redis();
  const phones = await r.zrange(CONV_SET, 0, SEARCH_POOL - 1, { rev: true });
  if (!phones.length) {
    return { interested: 0, not_interested: 0, not_answering: 0, untagged: 0, sonia_calls: 0, reshma_calls: 0 };
  }

  const items = await hydrate(phones);
  const counts = { interested: 0, not_interested: 0, not_answering: 0, untagged: 0, sonia_calls: 0, reshma_calls: 0 };
  for (const c of items) {
    ...
    if (c.calledBy === 'Sonia') counts.sonia_calls++;
    else if (c.calledBy === 'Reshma') counts.reshma_calls++;
  }
  return counts;
}
