...
const UPSERT_CHUNK = 50; // leads per pipelined round trip

/**
 * Batched version of upsertLead — pipelines writes in chunks instead of one
 * sequential round trip per lead. A single lead does ~4-5 Redis commands, so
 * a few hundred leads processed one-at-a-time (as CSV batches and the Zoho
 * sync page-loop both can be) can take tens of seconds of pure network
 * latency and get killed mid-batch by Vercel's function timeout — this is
 * the fix for that (found via a real stuck-mid-sync live test, not
 * hypothetical).
 * @returns {Promise<{imported:number, invalid:string[]}>}
 */
export async function upsertLeadsBatch(leads, defaultListId) {
  const r = redis();
  const now = Date.now();
  const invalid = [];
  let imported = 0;
  ...
  for (let i = 0; i < leads.length; i += UPSERT_CHUNK) {
    ...
    for (const lead of chunk) {
      const phone = normalizePhone(lead.phone);
      if (!phone) {
        invalid.push(lead.phone);
        continue;
      }
      const tags = (lead.tags || []).map(String).filter(Boolean);
      const listId = lead.listId || defaultListId || '';

      pipeline.hset(k.lead(phone), {
        phone,
        name: lead.name || '',
        source: lead.source || '',
        tags: JSON.stringify(tags),
      ...
      pipeline.zadd(LEADS_ALL, { score: now, member: phone });
      if (lead.source) pipeline.sadd(k.bySource(lead.source), phone);
      for (const t of tags) pipeline.sadd(k.byTag(t), phone);
      if (listId) pipeline.sadd(k.byList(listId), phone);
      queued++;
    }
...
// Upstash auto-deserializes JSON-shaped hash values back into real
// objects/arrays on read — already-an-object needs no parsing, only a
// string does. (Same class of surprise as the numeric-string coercion
// handled elsewhere; this is its JSON-value counterpart.)
...
function parseLead(phone, h) {
  ...
  const tags = parseJsonField(h.tags, []);
  const extra = parseJsonField(h.extra, {});
  return {
    phone: String(phone),
    name: h.name || '',
    source: h.source || '',
