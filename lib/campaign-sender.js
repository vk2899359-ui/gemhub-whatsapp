...
async function attemptSend(campaign, phone, attemptsSoFar) {
  ...
  try {
    ...
    await setRecipientStatus(campaign.id, phone, { status: 'sent', sentAt: Date.now(), attempts: attemptsSoFar + 1 });
...
// Fire-and-forget POST to our own tick endpoint. Uses the campaign secret
// for auth (same trust boundary as the cron endpoints elsewhere in this
// app). Delaying when idle avoids a tight busy-loop while every recipient
// is mid-backoff.
...
async function selfChain(delayMs) {
  ...
  if (delayMs > 0) {
    ...
    // Don't await — let the current response return immediately; caller
    // wraps this whole function in waitUntil so it still completes.
    fire();
  }
}
