...
/** 'IMAGE' | 'VIDEO' | null — whichever media format this template's header needs. */
function headerMediaFormat(templateComponents) {
  const header = (templateComponents || []).find((c) => c.type === 'HEADER');
  if (header?.format === 'IMAGE' || header?.format === 'VIDEO') return header.format;
  return null;
}
...
export function buildSendComponents(campaign, lead) {
  const components = [];
  const mediaFormat = headerMediaFormat(campaign.templateComponents);
  if (campaign.imageMediaId && mediaFormat) {
    const key = mediaFormat === 'VIDEO' ? 'video' : 'image';
    components.push({ type: 'header', parameters: [{ type: key, [key]: { id: campaign.imageMediaId } }] });
  }
  const n = bodyVarCount(campaign.templateComponents);
  if (n > 0) {
    const params = [];
