...
const IMAGE_MESSAGE_HINT = `[Context: The customer just sent a photo, most likely a jewellery piece they saw somewhere (Instagram, Pinterest, another store, a friend's ring) and want to know if GemHub has something similar, or a photo of their own hand/finger for sizing, or a screenshot of a GemHub product. Describe what you can see (style, stone shape, setting type, metal tone) in one short line, then use lookup_products. When candidates come back, lookup_products also attaches each one's actual product photo — genuinely LOOK at those photos side-by-side with the customer's photo before deciding. A product whose title merely sounds similar is NOT the same as one that visually matches — judge the actual photo, not just the name. Only say "we don't have this" after you've actually checked the returned photos and none of them are a real style match; even then, try one broader lookup_products search (different keywords) before concluding that — this is very often an EXISTING catalogue piece, not a reason to jump straight to offering a custom design.]`;
